document.addEventListener('DOMContentLoaded', () => {
    ...
    const init3DScene = () => {
        ...
        const orbitRings = [
            { radius: radius + 32, color: 0xff007f, opacity: 0.2, tilt: 0.18 },
            { radius: radius + 52, color: 0x00f3ff, opacity: 0.13, tilt: -0.42 },
            { radius: radius + 72, color: 0xff5f00, opacity: 0.1, tilt: 0.72 }
        ];

        orbitRings.forEach((orbit, index) => {
            const orbitMesh = new THREE.Mesh(
                new THREE.TorusGeometry(orbit.radius, index === 0 ? 0.8 : 0.55, 8, 96),
            ...
            orbitMesh.rotation.x = Math.PI / 2 + orbit.tilt;
            orbitMesh.rotation.y = orbit.tilt * 0.7;
            orbitGroup.add(orbitMesh);
        });

        const nodeGeometry = new THREE.SphereGeometry(2.8, 10, 10);
        const nodeMaterials = [
            new THREE.MeshBasicMaterial({ color: 0xff007f, transparent: true, opacity: 0.9 }),
        ...
        for (let i = 0; i < 12; i++) {
            ...
            const nodeRadius = radius + 42 + (i % 3) * 17;
            node.position.set(
                Math.cos(angle) * nodeRadius,
                Math.sin(angle * 1.35) * (nodeRadius * 0.65),
        ...
        const tick = () => {
            ...
            orbitGroup.rotation.x = Math.sin(elapsedTime * 0.35) * 0.08;
            core.rotation.x = elapsedTime * 0.28;
            core.rotation.y = -elapsedTime * 0.42;
            core.scale.setScalar(1 + Math.sin(elapsedTime * 1.8) * 0.08);

            orbitNodes.forEach((node, index) => {
                const pulse = 1 + Math.sin(elapsedTime * 2.2 + index) * 0.22;
